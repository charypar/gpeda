...
%   OPTARG1     directory where local files should be placed ("scratchdir" at metacentrum)
  ...
  for dim = bbParams.dimensions            % small dimensions first, for CPU reasons
    ...
    for ifun = bbParams.functions          % or benchmarksnoisy(...)
      ...
      gnuplotScriptCommand = ['sed "s#\<DATAFILE\>#' gnuplotFile '.dat#; s#\<OUTPUTFILE\>#' resultsFile '#; s#\<TITLE\>#f' num2str(ifun) ', ' num2str(dim) 'D#; s#\<DATALINETITLE\>#' modelType ' MGSO#; s#\<PARAMS1\>#' sprintfStruct(surrogateParams, 'escape') '#; s#\<PARAMS2\>#' sprintfStruct(exp_settings, 'escape') '#" ' gnuplotScript ' > ' gnuplotFile '.gpi'];
