function [rnd, neval] = rayslicesample(initial,nsamples,varargin)
...
RN = randn(nsamples*thin+burnin,dim); % gaussian normal random numbers for samples on sphere
RD = rand(nsamples*thin+burnin,1); % uniformly draw the point within the slice
...
for i = 1-burnin:nsamples*thin
    ...
    % pick a random point on dim-dimensional sphere
    rgauss = RN(i+burnin,:);
    rand_sphere = 1/sqrt(sum(rgauss.^2))*rgauss;
    
    % define a step of specified width in this direction
    step = (rand_sphere .* width);
    xl = x0 - RD(i+burnin)*step;
    xr = xl + step;
    iter = 0;
    
    % step out procedure is performed in these directions
    ...
    while(~inside(xp,z))&& iter<maxiter 
        ...
        xp = rand(1) * (xr-xl) + xl; % draw again, in the original 1D direction
        iter = iter+1;
    end

    % plot([xl(1) xr(1)],[xl(2) xr(2)],'g-');
