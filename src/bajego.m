function [xopt varargout] = bajego(options, eval_fcn, doe, rescale, restart, stop, varargin)
% Gausian Process based Jones' EGO (Efficient Global Optimization) algorithm. Performs a
...
% rescale - rescale conditions (function handle or cell array of function handles), e.g. stall
...
%
% Some of the strategies (restart & stop conditions) can have composite structure: multiple
% function handles in a cell array. In that case they are used together (reduced with 
% logical OR). Respective field in the options struct should 
...
while ~evalconds(stop, run, options.stop) % until one of the stop conditions occurs
  ...
  if (nTrainErrors > maxTrainErrors) 
    ...
    if (~isfield(run.attempts{att}.model, 'dataset'))
      % the actual model does not have any dataset yet, take the very last model anyway
      M = M_try;
    end
    ...
    disp(['Creating a new individual, generation ' int2str(it) '...']);
    try
      % do a EI maximization instead of sampling
      ei_max = findEIMaximum(M, run.attempts{att}, dim);
      
      if (isCovarianceSPD(M, [pop; ei_max], 'bool'))
        disp('Augmenting dataset with EI''s maximum.');
        if (size(pop,1) == options.popSize)
          [minx mini] = min(distToDataset(ei_max, pop));
          new_pop_i = [1:(mini-1) (mini+1):size(pop,1)];
          pop = [pop(new_pop_i,:); ei_max];
        else
          pop = [pop; ei_max];
        end
      end
...
function [pop fval exflag] = findEIMaximum(model, thisAttempt, dim)
  model_EI = @(x) - modelGetEI(model, x, thisAttempt.bests.yms2(end,1));
  ...
  fminoptions = optimset('MaxFunEvals', min(1000*dim), ...
    ...
    'TolX', 1e-10, ...
    'Display', 'off');
  [pop fval exflag output] = fminsearch(model_EI, thisAttempt.bests.x(end,:), fminoptions);
