function [xopt varargout] = gpedaGPS(options, eval_fcn, doe, sampler, rescale, restart, stop, varargin)
...
while ~evalconds(stop, run, options.stop) % until one of the stop conditions occurs
  ...
  % train model
  disp('Training model...'  );
  [M_try nTrainErrors] = modelTrainGPS(M, ds.x, ds.y);
...
function [pop fval exflag] = findModelMinimum(model, thisAttempt, dim)
  gp_predict = @(x_gp) modelPredictGPS(model, x_gp);
